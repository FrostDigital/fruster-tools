...
function create(path, name, branch) {	
	...
	if(fileSvcRegClient.isValid(path)) {
		...
		serviceRegistryModelPromise = gitSvcRegClient.get(path);
	} else {
		throw new Error("Invalid path: " + path);
	}

	return serviceRegistryModelPromise.then(serviceRegistryModel => new ServiceRegistry(serviceRegistryModel, name, branch));	
}
