...
class ServiceRegistry {
    ...
    constructor(model, options = {}) {
        ...
        this.services = model.services
            .filter((service) => service.name !== options.exclude)
            .map(service => {
                service.isDir = this.isDir(service);
                service.workDir = service.isDir ? service.repo : this.getWorkdir(service);
                service.env = this.interpolateEnv(Object.assign({}, service.env, model.env), options.passHostEnv);
                return service;
            });
    ...
    build(onData) {
        this.buildProcesses = this.services.map(service => {
            console.log("\n");
            console.log("BUILD");
            console.log(require("util").inspect(service, null, null, true));
            console.log("\n");
            return spawn({
                name: service.name,
    ...
    interpolateEnv(env, passHostEnv) {
        for (let k in env) {
            let envValue = env[k];
