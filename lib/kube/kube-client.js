 ...
 */
const createDeployment = async (namespace, serviceConfig, dryRun = false) => {
 ...
 * @param {string} serviceName
 */
const getConfig = async (namespace = null, serviceName) => {
	try {
		/** @type {any} */
		let body;
		...
		if (namespace) {
			...
			const qs = { qs: { labelSelector: "fruster=true,app=" + serviceName } };
			const { body: oBody } = await client.api.v1.secret.get(qs);

			if (oBody.items.length > 1) {
				throw new Error(
					`Found multiple configurations for ${serviceName}, consider using -n to set a specific namespace`
				);
			}
...
//  * @param {string} serviceName
//  */
// const getLogs = async (namespace, serviceName) => {
// 	try {
// 		const { body } = await client.api.v1
// 			.namespace(namespace)
// 			.po(serviceName)
