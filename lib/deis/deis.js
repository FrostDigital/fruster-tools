...
deis.createApp = (appName, remoteName) => {
  if(appName.length > 24) {    
    return Promise.reject("App name cannot be more than 24 chars");
  }
  return cmd(`deis create ${appName} ${remoteName ? "-r " + remoteName : "--no-remote"}`, false);
};

deis.setConfig = (app, config, asJson) => {
  if(asJson) {
...
deis.getConfig = (app, asJson = false) => {
  if(asJson) {
    return doRequest("GET", `/v2/apps/${app}/config`)
