...
module.exports = {
	start: function(serviceRegistryPath, options = {}) {
		return serviceReg.create(serviceRegistryPath, options)
			.then(serviceRegistry => {
				return new Runner(serviceRegistry, options).run();
			})
			.catch(err => {
				console.log("Failed building/starting services", err);
				process.exit(1);
			});
	}
};
...
class Runner {
	constructor(serviceRegistry, options = {}) {
		...
		this.input = "";
		this.skipUpdate = options.skipUpdate;
		this.verboseOutput = options.verboseOutput;
		this.skipBuild = options.skipBuild;
		this.skipStart = options.skipStart;	
		this.allowBuildFailures = options.allowBuildFailures;	
	}

	async run() {	
		if (!this.skipUpdate) {
			await this.serviceRegistry.cloneOrUpdateServices();			
		}
