...
async function startBox(serviceRegPath, dockerImage) {
	...
	const serviceReg = await serviceRegistryFactory.create(serviceRegPath, {
	...
	const servicesJsonFilePath = utils.writeTempJsonFile(
		"services.json",
		serviceReg.toJSON()
	);
	...
	const createOpts = {
		...
		Cmd: [
			"/bin/bash",
			"-c",
			`(/root/start.sh &) && fruster run /service-registry/services.json --dir /services --skip-build --skip-update --exclude ${
				program.exclude
			}`
		],
		HostConfig: {
