...
`
	...
	.option(
		"-p, --prune",
		"remove config from apps that is not defined in service registry"
	)
	...
	.option(
		"-l, --set-limits",
		"set resource limits from service registry or fallback to default"
	)
...
async function applyConfig() {
	const serviceRegistry = await serviceRegistryFactory.create(
		serviceRegPath,
		{
			passHostEnv: passEnv,
	...
	for (let i = 0; i < serviceRegistry.services.length; i++) {
		const service = serviceRegistry.services[i];
		...
		if (!apps.find(app => app.id == service.name)) {
			if (createApps) {
				...
				if (!dryRun) {
					await deis.createApp({ appName: service.name });
				}
			} else {
				log.warn(
					`Service ${
						service.name
					} does not exist in deis, skipping config of this service`
				);
				service.skip = true;
			}
		}
		...
		if (addHealthcheck && !service.skip) {
			...
			if (!dryRun) {
				await deis.enableHealthcheck(service.name);
			}
		}
	}
	...
	let changeSetPromises = services.map(async service => {
		const existingConfig = await deis.getConfig(service.name);
		...
		for (let k in existingConfig) {
			if (service.env[k] === undefined) {
				if (prune) {
					log.warn(
						`[${service.name}] Will remove ${k} (value was "${
							existingConfig[k]
						}")`
					);
					changeSet[k] = null;
				...
				console.log(
					`[${service.name}] Updating ${k} ${existingConfig[k]} -> ${
						service.env[k]
					}`
				);
		...
		for (let k in service.env) {
			if (existingConfig[k] === undefined) {
				console.log(
					`[${service.name}] New config ${k}=${service.env[k]}`
				);
				changeSet[k] = service.env[k];
			}
		}
	...
	return Promise.all(changeSetPromises).mapSeries(async changeSet => {
		if (!dryRun && changeSet.changeSet) {
			console.log(`[${changeSet.serviceName}] Updating config...`);

			try {
				await deis.setConfig(
				...
				log.error(
					`[${
						changeSet.serviceName
					}] got error while updating config:\n${err.message}`
				);
			}
		}
	});
